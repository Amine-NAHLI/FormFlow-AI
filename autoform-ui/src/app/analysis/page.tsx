      ...
      <main className="max-w-6xl mx-auto p-6 md:p-10 flex flex-col gap-8">
        ...
        <div className="bg-white rounded-2xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] p-8 border border-slate-100 flex flex-col md:flex-row gap-8">
          ...
          <div className="flex-1 bg-[#F4F0FF] rounded-xl p-6 border border-[#E9E1FE] flex flex-col justify-center">
            ...
            <p className="text-sm text-[#5b38d1] leading-relaxed mb-4">
              Les calculs sont effectués localement avec des formules mathématiques exactes (Pearson, OLS, écart-type). L&apos;IA ne fait que rédiger l&apos;interprétation textuelle.
            </p>
            <ul className="text-sm text-[#5b38d1] space-y-2 font-medium">
              <li>✅ Calculs 100% déterministes (identiques à SPSS)</li>
              <li>✅ Corrélations de Pearson exactes</li>
              <li>✅ Régression linéaire (Moindres Carrés)</li>
              <li>✅ Rapport interprété par l&apos;IA</li>
            </ul>
          </div>
        </div>
        ...
        {isLoading && (
          <div className="w-full h-64 bg-white rounded-2xl flex flex-col items-center justify-center gap-4 animate-pulse">
            ...
            <p className="text-slate-500 font-semibold">Calcul des statistiques et rédaction du rapport...</p>
          </div>
        )}
          ...
          <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            {results.meta && (
              <div className="flex flex-wrap gap-3">
                <span className="px-4 py-2 bg-white rounded-xl border border-slate-100 text-sm font-semibold text-slate-700 shadow-sm">{results.meta.totalRows} réponses</span>
                <span className="px-4 py-2 bg-white rounded-xl border border-slate-100 text-sm font-semibold text-slate-700 shadow-sm">{results.meta.totalColumns} colonnes</span>
                <span className="px-4 py-2 bg-[#F4F0FF] rounded-xl border border-[#E9E1FE] text-sm font-semibold text-[#6D44F1] shadow-sm">{results.meta.numericColumns} numériques</span>
                <span className="px-4 py-2 bg-[#F4F0FF] rounded-xl border border-[#E9E1FE] text-sm font-semibold text-[#6D44F1] shadow-sm">{results.meta.categoricalColumns} catégorielles</span>
              </div>
            )}
            ...
            <div className="bg-white p-8 rounded-2xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-slate-100">
              <h2 className="text-xl font-bold text-slate-800 mb-4 flex items-center gap-2">
                <svg className="w-6 h-6 text-[#6D44F1]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"></path></svg>
                Rapport du Data Analyst (IA)
              </h2>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="bg-white p-6 rounded-2xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-slate-100">
                <h3 className="text-lg font-bold text-slate-800 mb-6 flex items-center gap-2">
                  ...
                  Statistiques Descriptives (Calcul Exact)
                </h3>
                <div className="space-y-4">
                  {results.statistics?.map((stat: {name: string; value: string; interpretation: string}, i: number) => (
                    <div key={i} className="flex flex-col p-4 bg-slate-50 rounded-xl border border-slate-100">
                      ...
                      <span className="text-lg font-black text-[#6D44F1] my-1 font-mono">{stat.value}</span>
              ...
              <div className="bg-white p-6 rounded-2xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-slate-100">
                ...
                <div className="space-y-4">
                  {results.correlations?.map((corr: {var1: string; var2: string; coefficient: number; significance: string}, i: number) => (
                    <div key={i} className="flex flex-col p-4 bg-slate-50 rounded-xl border border-slate-100">
                      ...
                      <div className="flex items-center justify-between">
                        <span className="text-sm font-medium text-slate-600">r = <span className="font-bold text-slate-800 font-mono">{corr.coefficient}</span></span>
                  ...
                  {(!results.correlations || results.correlations.length === 0) && (
                    <p className="text-sm text-slate-400 text-center py-4">Aucune corrélation numérique détectée.</p>
                  )}
                </div>
              </div>
            </div>
            ...
            {results.regression && (
              <div className="bg-white p-6 rounded-2xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-slate-100">
                <h3 className="text-lg font-bold text-slate-800 mb-4 flex items-center gap-2">
                  <svg className="w-5 h-5 text-[#6D44F1]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M13 7h8m0 0v8m0-8l-8 8-4-4-6 6"></path></svg>
                  Régression Linéaire (OLS)
                </h3>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div className="p-4 bg-slate-50 rounded-xl border border-slate-100 text-center">
                    <span className="text-xs font-bold text-slate-400 uppercase block mb-1">Équation</span>
                    <span className="text-lg font-black text-[#6D44F1] font-mono">y = {results.regression.slope}x + {results.regression.intercept}</span>
                  </div>
                  <div className="p-4 bg-slate-50 rounded-xl border border-slate-100 text-center">
                    <span className="text-xs font-bold text-slate-400 uppercase block mb-1">R² (Qualité)</span>
                    <span className="text-2xl font-black text-[#6D44F1] font-mono">{results.regression.rSquared}</span>
                  </div>
                  <div className="p-4 bg-slate-50 rounded-xl border border-slate-100 text-center">
                    <span className="text-xs font-bold text-slate-400 uppercase block mb-1">Variables</span>
                    <span className="text-sm font-semibold text-slate-700">{results.regression.xVar} → {results.regression.yVar}</span>
                  </div>
                </div>
              </div>
            )}
            ...
            {results.correlations && results.correlations.length > 0 && (
              <div className="bg-white p-6 rounded-2xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-slate-100 h-80">
                <h3 className="text-lg font-bold text-slate-800 mb-6">Visualisation des Corrélations</h3>
                <ResponsiveContainer width="100%" height="85%">
                  <BarChart data={results.correlations.map((c: {var1: string; var2: string; coefficient: number}) => ({ name: `${c.var1.substring(0,10)}/${c.var2.substring(0,10)}`, value: c.coefficient }))}>
                    <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#f1f5f9" />
                    <XAxis dataKey="name" axisLine={false} tickLine={false} tick={{fill: '#94a3b8', fontSize: 11}} />
                    <YAxis axisLine={false} tickLine={false} tick={{fill: '#94a3b8', fontSize: 12}} domain={[-1, 1]} />
